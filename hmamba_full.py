...
from mamba.mamba_ssm.modules.mamba_simple import Mamba,Arcosh
...
class Mamba4Rec(SequentialRecommender):
	def __init__(self, config, dataset):
		...
		# import pdb
		# pdb.set_trace()
		# print("--check--:", self.item_embedding.state_dict()['weight'])

		self.LayerNorm = nn.LayerNorm(self.hidden_size+1, eps=1e-12)
		self.dropout = nn.Dropout(self.dropout_prob)
		self.max_seq_length = config["MAX_ITEM_LIST_LENGTH"]
		# self.position_embedding = nn.Embedding(
  #           self.max_seq_length + 1, self.hidden_size)
		# self.position_embedding = nn.Embedding(
		#     self.max_seq_length + 1, self.hidden_size+1)  # add mask_token at the last
		...
		# ])
		# self.position_embedding = nn.Linear(
		# 	1, self.hidden_size)  # add mask_token at the last
		...
		# self.a = torch.nn.Parameter(torch.zeros(self.n_items,))
		# self.z = torch.nn.Parameter(F.pad(torch.zeros(self.n_items, self.hidden_size-2), pad=(1,0), value=1)) # z should not be (0,0)

		# self.init_weights1()
		# self.acti = nn.Sigmoid()
		# self.item_embedding.weight = self.item_embedding.state_dict()['weight']
		# import pdb
		# pdb.set_trace()
		# self.item_embedding.weight = nn.Parameter(self.mamba_layers.mamba.expmap0(self.item_embedding.state_dict()['weight']))
		# println()
		# self.mamba_layers = nn.ModuleList([self.mamba] for _ in range(self.num_layers))
		# self.mamba_layers = nn.ModuleList([
		...
		self.l2 = nn.Linear(65, 64)
		
		
		self.actip= nn.Sigmoid()
  		# self.l3 = nn.Linear(65, 64)
		# self.position_embedding = nn.Sequential(
        #     nn.Conv1d(1, 32, kernel_size=1, padding='same'), #Padding to maintain length
        #     nn.GELU(),
        #     nn.Conv1d(32, 64, kernel_size=1, padding='same'), #Another layer to increase features
        #     nn.Sigmoid()
        # )
	...
	def _init_weights(self, module):
		if isinstance(module, (nn.Linear, nn.Embedding)):
			...
			# module.bias.data.xavier_uniform_()
			# module.weight.data.xavier_uniform_()
		if isinstance(module, nn.Linear) and module.bias is not None:
			module.bias.data.zero_()

	
	def poincare_ball_projection(self,data, c=1.0):
		norm = np.linalg.norm(data, axis=1, keepdims=True)
		return data / (1 + np.sqrt(1 + c * norm**2))
	...
	def forward(self, item_seq, item_seq_len):
		...
		input_emb_p1 = self.item_embedding(item_seq)
		# input_emb_p1 = input_emb_p1 + llm_output
		o = torch.zeros_like(input_emb_p1)
		x = torch.cat([o[:, :, 0:1], input_emb_p1], dim=-1)
		np.save('./dataset/embeb_elu.npy', x.cpu().detach().numpy())

		# import pdb
		# pdb.set_trace()
		# x_after = self.poincare_ball_projection(x.cpu().detach().numpy())
		input_emb = self.mamba_layers.mamba.expmap0(x)
		np.save('./dataset/embeb_hyper.npy', input_emb.cpu().detach().numpy())
		# import pdb
		# pdb.set_trace()
		nor_res = torch.norm(input_emb, p=2.0, dim=-1)
		# print("check:",nor_res.size(), torch.mean(nor_res))
		...
		# for i in range(self.num_layers):
			...
			# input_emb = torch.exp(input_emb+1e-6)

			# item_emb = self.mamba_layers[i](input_emb)
			# import pdb
			# pdb.set_trace()
			# item_emb = self.mamba_layers(input_emb)
			# item_emb = self.mamba_layers[i](item_emb)
			# print("check item:", len(item_emb), item_emb.size(),item_emb[0][:10])
			# println()
		# import pdb
		# pdb.set_trace()
		# input_emb = self.mamba_layers.mamba.expmap(input_emb)
		item_emb = self.mamba_layers(input_emb)
		
		x_t = ((item_emb **2).sum(dim=-1)+1).sqrt().unsqueeze(-1)
		x_s = item_emb
		
		x_com = torch.cat([x_t, x_s], dim=-1)
		# x_com = self.l2(x_com)
		# item_emb = self.mamba_layers.mamba.logmap0(x_com)
		# print("--com:", x_com.size())
		seq_output = self.gather_indexes(x_com, item_seq_len - 1)  #torch.Size([1, 65])
		# print("--com:", seq_output.size())
		return [seq_output,input_emb,torch.mean(nor_res)]
	...
	def calculate_loss(self, interaction):
		...
		if self.loss_type == "BPR":
			...
			pos_score = self.mamba_layers.mamba.compute_dist(x= seq_output[1], y = seq_output[0].unsqueeze(1))
			loss = pos_score + 0.1
			loss[loss < 0] = 0
			loss = torch.mean(torch.sum(loss, dim=-1))
			return loss
		else:  # self.loss_type = 'CE'
			# Elucidean space loss
			test_item_emb = self.item_embedding.weight.unsqueeze(0)
			# test_item_emb = self.item_embedding.weight
			o = torch.zeros_like(test_item_emb)
			
			x = torch.cat([o[:, :, 0:1], test_item_emb], dim=-1)
			# import pdb
			# pdb.set_trace()
			test_item_hyper = (self.mamba_layers.mamba.expmap0(x)).squeeze(0)
			# logits = torch.matmul(seq_output[0], test_item_emb.transpose(0, 1))  #logits
			# import pdb
			# pdb.set_trace()
			logits = (self.mamba_layers.mamba.compute_dist(x= seq_output[0], y = test_item_hyper.transpose(0,1))).squeeze(-1)
			# print("---logits:---", logits.size(), torch.mean(logits), pos_items.size())
			# println()
			
			loss = self.loss_fct(logits, pos_items)
			return loss, seq_output[2]
	...
	def predict(self, interaction):
		...
		seq_output = self.forward(item_seq, item_seq_len)
		test_item_emb = self.item_embedding(test_item).unsqueeze(0)
		o = torch.zeros_like(test_item_emb)
		x = torch.cat([o[:, :, 0:1], test_item_emb], dim=-1)
		# import pdb
		# pdb.set_trace()
		...
		test_item_hyper = (self.mamba_layers.mamba.expmap0(x)).squeeze(0)
		# logits = torch.matmul(seq_output[0], test_item_emb.transpose(0, 1))
		# print("**:", test_item_emb.size())
		# import pdb
		# pdb.set_trace()
		scores = (self.mamba_layers.mamba.compute_dist(x= seq_output[0], y = test_item_hyper.transpose(0,1))).squeeze(-1)

		# scores = torch.mul(seq_output[0], test_item_emb).sum(dim=1)  # [B]
		return scores
	...
	def full_sort_predict(self, interaction):
		...
		# scores = torch.matmul(
		# 	seq_output[0], test_items_emb.transpose(0, 1)
		# )  # [B, n_items]
		test_item_emb = self.item_embedding.weight.unsqueeze(0)
		o = torch.zeros_like(test_item_emb)
		x = torch.cat([o[:, :, 0:1], test_item_emb], dim=-1)
		...
		# logits = torch.matmul(seq_output[0], test_item_emb.transpose(0, 1))  #logits
		scores = (self.mamba_layers.mamba.compute_dist(x= seq_output[0], y = test_item_hyper.transpose(0,1))).squeeze(-1)
		# scores = self.ce_logits(seq_output[0])
