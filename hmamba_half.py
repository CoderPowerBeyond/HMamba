...
class Mamba4Rec(SequentialRecommender):
  ...
  #           self.max_seq_length + 1, self.hidden_size)
	...
	def forward(self, item_seq, item_seq_len):
		...
		# import pdb
		# pdb.set_trace()
		# position_embedding = self.actip(self.position_embedding(item_seq.unsqueeze(-1).float()))
		# position_embedding = self.position_embedding(item_seq.unsqueeze(-1).transpose(-1,-2).float()).transpose(-1,-2)

		input_emb_p1 = self.item_embedding(item_seq)
		# input_emb_p1 = position_embedding + input_emb_p1
		...
		seq_output = self.gather_indexes(item_emb, item_seq_len - 1)  #torch.Size([1, 65])
		# print("--com:", seq_output.size())
		return [seq_output,input_emb]
	...
	def calculate_loss(self, interaction):
		...
		if self.loss_type == "BPR":
			...
			test_item_emb = torch.cat([o[:, 0:1], test_item_emb], dim=-1)
			# import pdb
			# pdb.set_trace()
	...
	def predict(self, interaction):
		...
		# pdb.set_trace()
		# test_item_hyper = (self.mamba_layers.mamba.expmap0(x)).squeeze(0)
		scores = torch.matmul(seq_output[0], test_item_emb.transpose(0, 1))
	...
	def full_sort_predict(self, interaction):
		...
		o = torch.zeros_like(test_items_emb)
		test_items_emb = torch.cat([o[:, 0:1], test_items_emb], dim=-1)
		scores = torch.matmul(
